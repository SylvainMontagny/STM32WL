...
void PrintKey( KeyIdentifier_t key );
...
void PrintKey( KeyIdentifier_t key )
{
    ...
    if (retval == SECURE_ELEMENT_SUCCESS)
    {
        ...
        {
            MW_LOG(TS_OFF, VLEVEL_L, "> AppSKey:                ");
        }
        else if (key == NWK_S_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "> NwkSKey:                ");
        }
        else if (key == MC_ROOT_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "###### MCRootKey:   ");
        }
        else if (key == MC_KE_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "###### MCKEKey:     ");
        }
        else if (key == MC_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "###### MCKey_0:     ");
        }
        else if (key == MC_APP_S_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "###### MCAppSKey_0: ");
        }
        else if (key == MC_NWK_S_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_L, "###### MCNwkSKey_0: ");

        }
        MW_LOG(TS_OFF, VLEVEL_L, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
...
SecureElementStatus_t SecureElementInit( SecureElementNvmData_t *nvm, SecureElementGetUniqueId seGetUniqueId )
{
    ...
    //PrintKey(APP_S_KEY);
    //PrintKey(NWK_S_KEY);
    //MW_LOG(TS_OFF, VLEVEL_M, "###### MC  ######\r\n");
    //PrintKey(MC_ROOT_KEY);
    //PrintKey(MC_KE_KEY);
    //PrintKey(MC_KEY_0);
    //PrintKey(MC_APP_S_KEY_0);
    //PrintKey(MC_NWK_S_KEY_0);
    //MW_LOG(TS_OFF, VLEVEL_M, "###### Device  ######\r\n");


    if ((VLEVEL_L == 1) && (KEY_EXTRACTABLE == 1)){

    	if(ADMIN_ACTIVATION_TYPE == 2){ //OTAA
    	    // PrintKey(APP_KEY);
    	    PrintKey(NWK_KEY);
    	}
    	if(ADMIN_ACTIVATION_TYPE == 1){ //ABP
    	    PrintKey(APP_S_KEY);
    	    PrintKey(NWK_S_KEY);
    	}
    }
